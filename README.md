...
> These files are pre-generated to make the client aware of file size that should be received (for simplicity). 
...
This script by default should be executed in the parent directory of the workloads. The workloads are generated using random generator based on the size of the spark example of LDA application. Each workload is then doubled to increase the size. E.g., Size of the file "broadcast_0" in "broadcast_files_2" is 2x the size of "broadcast_0" in "broadcast_files_1".  
...
```
 ...
 ## Workload files
The files provided in this repo contain the list of the actual files to be transmitted and their sizes (pickle output). 
The actual files are stored on the 2TB SSD at ```./orca-files/workloads/```.


The size of workload files and number of the rounds are based on the Spark [LDA example here](https://cs-git-research.cs.surrey.sfu.ca/nsl/DataCenterMulticast/sparkdocs/-/blob/master/Complete_setup_steps.txt#L109). The broadcast files are generated by spark and can be found in the ramdisk location (in Spark examples it was ```/mnt/ramdisk```).
# Plot Results
